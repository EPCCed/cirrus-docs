...
are making an experimental installation tailored for Cirrus available
centrally. This page provides documentation on how to activate and install
packages using the central installation on Cirrus. For more in-depth
...
Spack is configured using YAML files. The central installation on Cirrus made
...
Your own configuration should fit in the user level scope. On Cirrus Spack is
...
Spack defaults which are overridden by the Cirrus system configuration files,
...
`cirru-user` directory which is where we have told Spack how to use packages
...
able to use these with no issues on Cirrus by simply running `spack install` as
...
`$SPACK_ROOT/var/spack/repos/cirrus`.
...
also by the `cirrus` repository; the latter has been patched to work on
Cirrus.
...
`builtin` and `cirrus` use the same directory name and namespace.
...
for example `builtin.bison@3.8.2` and `cirrus.quantum-espresso@7.2`. In order
...
This means that `cirrus` has higher priority. Because of this, running `spack
install quantum-espresso` would install `cirrus.quantum-espresso`, but you
...
ScaLAPACK on Cirrus. These are bundled together into single libraries named for
